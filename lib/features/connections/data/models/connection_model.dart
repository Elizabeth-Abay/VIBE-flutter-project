...
class ConnectionRepository {
  ...
  Future<bool> acceptRequest(String requesterId) async {
    try {
      await _api
          .post('/connections/accept', body: {'requester_id': requesterId});
  ...
  Future<List<ConnectedUser>> _getCachedConnectedUsers() async {
    final db = await _db.database;
    final rows = await db
        .query('connections', where: 'status = ?', whereArgs: ['connected']);
  ...
  Future<List<SentRequestUser>> _getCachedSentRequests() async {
    final db = await _db.database;
    final rows = await db
        .query('connections', where: 'status = ?', whereArgs: ['pending']);
    return rows
        .map((r) => SentRequestUser(
              userId: r['target_id'] as String,
              name: r['target_id'] as String,
              profileImage: '',
