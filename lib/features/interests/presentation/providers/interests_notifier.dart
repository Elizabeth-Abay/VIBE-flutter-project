...
class InterestsSavingNotifier extends Notifier<bool> {
  ...
  Future<bool> update(Map<String, String> userSelections) async {
    // Prevent double clicking while spinning
    if (state) return false;
    ...
    try {
      // Send payload down to data layer
      final bool savedInterest = await _repo.updateInterests(formattedList);
...
/// Tracks user mutations on the Edit screen before hitting "Done Editing / Save"
// final selectedVibesNotifierProvider = StateProvider.autoDispose<Map<String, int>>((ref) {
//   // Watch the full AsyncValue instead of forcing `.value` synchronously
//   final profileAsync = ref.watch(profileProvider);

//   return profileAsync.maybeWhen(
//     data: (currentProfile) {
//       if (currentProfile != null) {
//         return currentProfile.vibes.map(
//           (key, value) => MapEntry(key, int.tryParse(value) ?? 2),
//         );
//       }
//       return {};
//     },
//     orElse: () => {}, // Returns an empty map while loading or on error
//   );
// });
